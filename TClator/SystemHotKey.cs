...
namespace TClator
{
    public class SystemHotKey
    {
        ...
        public static bool RegHotKey(IntPtr hwnd, int hotKeyId, KeyModifiers keyModifiers, Keys key, string hotkey = "")
        {
            if (!RegisterHotKey(hwnd, hotKeyId, keyModifiers, key))
            {
                ...
                if (errorCode == 1409)
                {
                    MessageBox.Show("热键" + hotkey + "被占用 ！");
                }
                else
                {
                    MessageBox.Show("热键" + hotkey + "注册失败！错误代码：" + errorCode);
                }
                return false;
            }
            return true;
